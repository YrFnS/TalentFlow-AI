...
export default function AIChatbot({ source = "landing" }: AIChatbotProps) {
	...
	const getQuickActions = () => {
		...
		if (source === "candidate") {
			return [
				{
					label: cb.checkAppStatus,
					message: cb.checkAppStatusMsg,
					icon: FileCheck,
				},
				{ label: cb.interviewTips, message: cb.interviewTipsMsg, icon: Mic },
				{
					label: cb.processQuestions,
					message: cb.processQuestionsMsg,
					icon: HelpCircle,
				},
				{
					label: cb.upcomingInterviews,
					message: cb.upcomingInterviewsMsg,
					icon: Briefcase,
				},
			];
		}
		if (source === "company") {
			return [
				{
					label: cb.searchCandidates,
					message: cb.searchCandidatesMsg,
					icon: Users,
				},
				{
					label: cb.helpJobPosting,
					message: cb.helpJobPostingMsg,
					icon: FileText,
				},
				{
					label: cb.hrCompliance,
					message: cb.hrComplianceMsg,
					icon: HelpCircle,
				},
				{
					label: cb.hiringAnalytics,
					message: cb.hiringAnalyticsMsg,
					icon: BarChart3,
				},
			];
		}
		...
		return [
			{
				label: t.common.chatbot.quickFindJobs,
				message: "I want to find jobs. Can you help me search?",
				icon: Briefcase,
			},
			{
				label: t.common.chatbot.quickInterviewTips,
				message: "Can you give me some interview tips?",
				icon: Mic,
			},
			{
				label: t.common.chatbot.quickResumeHelp,
				message: "I need help with my resume. What should I include?",
				icon: FileText,
			},
			{
				label: t.common.chatbot.quickAppStatus,
				...
				<div className="w-[calc(100vw-3rem)] sm:w-[340px] h-[70vh] max-h-[520px] flex flex-col rounded-2xl border border-border/50 bg-background shadow-2xl overflow-hidden">
					...
					<div className="flex items-center justify-between px-4 py-3 bg-slate-900 text-white">
						...
						<div className="flex items-center gap-1">
							<Button
								...
								aria-label={t.common.chatbot.newChat}
							>
					...
					>
						...
						<Button
							...
							aria-label={t.common.chatbot.send}
