...
export default function ModelsTab({
  providers, activeProviderId, onActiveProviderChange, onFetchModels,
  onToggleModel, onSetDefaultModel, onDeleteModel, t,
}: ModelsTabProps) {
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <Select value={activeProviderId || ''} onValueChange={onActiveProviderChange}>
          ...
          <Button variant="outline" onClick={() => { if (activeProvider) onFetchModels(); }} className="gap-2">
            <RefreshCw className="w-4 h-4" />{t.ai.fetchModels}
          </Button>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="hidden sm:table-cell">{t.ai.contextLength}</TableHead>
                ...
                {activeProvider.models.map((model) => (
                  <TableRow key={model.id}>
                    ...
                    <TableCell className="text-center">
                      <Switch checked={model.isActive} onCheckedChange={(checked) => onToggleModel(model, checked)} />
                    </TableCell>
                    <TableCell className="text-center">
                      {model.isDefault ? (
                        ...
                        <Button variant="ghost" size="sm" onClick={() => onSetDefaultModel(model)} className="text-xs">
                          {t.ai.setDefault}
                        </Button>
                      )}
                    </TableCell>
                    <TableCell className="text-end">
                      <Button variant="ghost" size="sm" onClick={() => onDeleteModel(model)} className="text-destructive hover:text-destructive">
