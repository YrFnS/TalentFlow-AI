 ...
 */
async function verifyStripeWebhook(request: NextRequest): Promise<{ valid: boolean; body: string; error?: string }> {
  ...
  try {
    ...
    return { valid: false, body: '', error: `Webhook signature verification failed: ${err.message}` };
  }
}
      ...
      switch (type) {
        case 'checkout.session.completed': {
          ...
          if (!plan) {
            ...
            plan = await db.plan.findFirst({ where: { type: planType as any } });
          }
          if (!plan) break;
        ...
        case 'customer.subscription.deleted': {
          ...
          const delSub = await db.subscription.findUnique({ where: { companyId: delCompanyId } });
          if (delSub) {
            await db.subscription.update({
              where: { id: delSub.id },
              data: {
