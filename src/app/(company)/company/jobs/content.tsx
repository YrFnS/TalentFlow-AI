...
export default function JobsPage() {
	...
	const load = useCallback(async (refresh = false) => {
		...
		try {
			...
			if (!response.ok)
				throw new Error(
					await getApiErrorMessage(response, "Unable to load jobs"),
				);
	...
	async function archive(job: Job) {
		if (
			!confirm(
				`Archive “${job.title}”? Candidates will no longer see this job.`,
			)
		)
			return;
		setArchiving(job.id);
		try {
			...
			if (!response.ok)
				throw new Error(
					await getApiErrorMessage(response, "Unable to archive job"),
				);
			setJobs((current) =>
				current.map((item) =>
					item.id === job.id ? { ...item, status: "ARCHIVED" } : item,
				),
			);
			toast.success("Job archived");
		} catch (reason) {
			toast.error(
				reason instanceof Error ? reason.message : "Unable to archive job",
			);
		} finally {
			setArchiving(null);
		}
			...
			<div className="flex flex-wrap items-start justify-between gap-4">
				<div>
					...
					<p className="mt-1 text-sm text-muted-foreground">
						Create openings, review activity, and manage publication status.
					</p>
				</div>
			...
			{filtered.length === 0 ? (
				<Card>
					<CardContent className="py-16 text-center">
						...
						<p className="mt-1 text-sm text-muted-foreground">
							{query || status !== "all"
								? "Change the filters to see more jobs."
								: "Create the first job for this company."}
						</p>
						{canEdit && (
							...
							<CardContent className="space-y-4 p-5">
								...
								<div className="space-y-2 text-sm text-muted-foreground">
									...
									<p className="flex items-center gap-2">
										...
										{job._count.applications} applicants · {job.openings}{" "}
										openings
									</p>
									<p className="flex items-center gap-2">
