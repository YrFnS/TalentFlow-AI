...
import { Plus, Trash2, GripVertical, ChevronUp, ChevronDown, AlertTriangle } from 'lucide-react';
...
export default function ScreeningQuestionEditor({
  question, index, total, onUpdate, onRemove, onMove,
  onAddOption, onRemoveOption, onUpdateOption, t,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {index > 0 && (
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onMove('up')} title={screening.moveUp}>
            <ChevronUp className="w-4 h-4" />
          </Button>
        )}
        {index < total - 1 && (
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onMove('down')} title={screening.moveDown}>
            <ChevronDown className="w-4 h-4" />
          </Button>
        )}
        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={onRemove} title={screening.removeQuestion}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="grid gap-2">
          ...
          <Select value={question.questionType} onValueChange={(v) => onUpdate('questionType', v)}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="grid gap-2">
          <Label className="text-xs">{screening.required}/{screening.optional}</Label>
      ...
      {question.questionType === 'MULTIPLE_CHOICE' && (
        <div className="grid gap-2">
          <Label className="text-xs">{screening.typeMultipleChoice} {screening.questionType.split(' ')[0]}</Label>
            ...
            {question.options.map((opt, optIndex) => (
              <div key={optIndex} className="flex items-center gap-2">
                ...
                {question.options.length > 2 && (
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => onRemoveOption(optIndex)}>
          ...
          <Button variant="outline" size="sm" onClick={onAddOption} className="h-7 text-xs border-slate-300 text-blue-600">
      ...
      <div className="space-y-3">
        ...
        {question.isKnockout && (
          <div className="ms-7 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30 space-y-3">
            ...
            <div className="grid gap-2">
              <Label className="text-xs text-amber-700">{screening.disqualifyAnswer}</Label>
              {question.questionType === 'YES_NO' ? (
                <Select value={question.knockoutAnswer} onValueChange={(v) => onUpdate('knockoutAnswer', v)}>
