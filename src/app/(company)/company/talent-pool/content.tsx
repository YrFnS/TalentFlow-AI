...
import {
  Users, Plus, Mail, Star, Clock, Grid3X3, List, Eye, UserPlus, Search,
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => setCreatePoolOpen(true)}>
                        ...
                        <td className="p-3">
                          <div className="flex flex-wrap gap-1">
                            {candidate.skills.slice(0, 3).map(s => (
                              <Badge key={s} variant="outline" className="text-[9px] px-1 py-0 border-slate-200 text-blue-700">
                                {s}
                              </Badge>
                            ))}
                          </div>
      ...
      <ProfileDialog
        ...
        onAddToPool={(candidate) => { setProfileOpen(false); openAddToPool(candidate); }}
        onEngage={(candidate) => { setProfileOpen(false); openEngage(candidate); }}
        t={tp}
      />
    </div>
  );
