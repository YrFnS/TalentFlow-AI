...
import { Calendar, Clock, Settings2, CalendarClock, Plus, X, Zap, CheckCircle2, Copy, CheckCircle } from 'lucide-react';
...
interface SelfSchedulingTabProps {
  ...
  updateAvailabilitySlot: (index: number, field: string, value: string | number) => void;
  formatTime: (d: string) => string;
  t: Record<string, any>;
}

export default function SelfSchedulingTab({
  availability, setAvailability, schedulingSlots, slotsLoading,
  savingAvailability, generatingSlots, daysToGenerate, onDaysToGenerateChange,
  copiedLink, onSaveAvailability, onGenerateSlots, onCopyLink,
  addAvailabilitySlot, removeAvailabilitySlot, updateAvailabilitySlot, formatTime, t,
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="availability" className="space-y-6">
        <TabsList className="bg-muted/50">
          <TabsTrigger value="availability" className="gap-2"><Settings2 className="w-4 h-4" />{t.selfScheduling.availabilityTab}</TabsTrigger>
          <TabsTrigger value="slots" className="gap-2"><CalendarClock className="w-4 h-4" />{t.selfScheduling.slotsTab}</TabsTrigger>
        </TabsList>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex justify-end pt-2">
                <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={onSaveAvailability} disabled={savingAvailability}>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-3 ms-auto">
                  <div className="flex items-center gap-2">
                    ...
                    <Input type="number" min={1} max={30} value={daysToGenerate} onChange={(e) => onDaysToGenerateChange(e.target.value)} className="h-8 w-16" />
                  </div>
                  <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={onGenerateSlots} disabled={generatingSlots}>
                      ...
                      return (
                        <Card
                          ...
                          className={cn(
                            ...
                            isAvailable ? 'border-slate-200/30 hover:border-slate-400' : 'border-amber-200 dark:border-amber-800/30'
                          )}
                        >
                          <CardContent className="p-4">
