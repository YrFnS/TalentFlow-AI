...
export default function ScheduleInterviewDialog({
  open, onOpenChange, formType, onFormTypeChange, formDate, onFormDateChange,
  formTime, onFormTimeChange, formDuration, onFormDurationChange,
  formInterviewer, onFormInterviewerChange, formNotes, onFormNotesChange,
  submitting, onSchedule, t,
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Input type="time" value={formTime} onChange={(e) => onFormTimeChange(e.target.value)} />
            </div>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="Interviewer name" value={formInterviewer} onChange={(e) => onFormInterviewerChange(e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label className="text-sm font-medium">{t.interviews.notes}</Label>
            <Textarea placeholder={t.interviews.notes} value={formNotes} onChange={(e) => onFormNotesChange(e.target.value)} rows={3} />
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>{t.common.cancel}</Button>
          <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={onSchedule} disabled={!formDate || !formTime || submitting}>
