...
export default function InterviewDetailsSheet({
  interview, open, onOpenChange, onCancel, formatDate, formatTime, getInitials, t,
          ...
          <SheetHeader className="p-6 pb-4 border-b">
            <SheetTitle className="flex items-center gap-3">
              ...
              <div className="text-start">
                ...
                <p className="text-sm text-muted-foreground">{getTypeLabel(interview.type)} · {interview.durationMinutes} {t.interviews.minutes}</p>
              </div>
            </SheetTitle>
          </SheetHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg border border-border/50">
                <Avatar className="w-10 h-10">
                  <AvatarFallback className="bg-teal-100 text-blue-700">{getInitials(interview.application.candidate.user.name)}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="text-sm font-medium">{interview.application.candidate.user.name}</p>
                  <p className="text-xs text-muted-foreground">{interview.application.job.title}</p>
                </div>
              </div>
            </div>
            <Separator />
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg border border-border/50">
                  ...
                  <p className="text-sm font-medium">{formatDate(interview.scheduledAt)}</p>
                </div>
                <div className="p-3 rounded-lg border border-border/50">
                  <p className="text-xs text-muted-foreground">{t.interviews.time}</p>
                  <p className="text-sm font-medium">{formatTime(interview.scheduledAt)}</p>
                </div>
              </div>
              <Badge variant="outline" className={cn('text-xs', sConfig.color, sConfig.bgColor)}>
                    ...
                    <div key={idx} className="flex items-center justify-between p-3 rounded-lg border border-border/50">
                      <div className="flex items-center gap-2">
                        <Avatar className="w-7 h-7">
                          <AvatarFallback className="bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 text-[10px]">{getInitials(assignment.interviewer.name)}</AvatarFallback>
                        </Avatar>
            ...
            {(interview.location || interview.meetingLink) && (
              <>
                <div className="space-y-2">
                  ...
                  {interview.location && <div className="flex items-center gap-2 text-sm"><MapPin className="w-4 h-4 text-muted-foreground" />{interview.location}</div>}
                  ...
                  {interview.rating && (
                    <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-lg font-bold text-amber-600">{interview.rating}/5</span>
                      </div>
