...
import { Video, Phone, MapPin, Star, CheckCircle2, XCircle, PlayCircle } from 'lucide-react';
import type { Interview, StatusConfig, TypeConfig } from './types';
import { statusConfig, typeConfig } from './types';
import { getInitials } from '@/lib/utils';

const iconMap: Record<string, React.ElementType> = {
  Calendar, Video, Phone, MapPin, CheckCircle2, XCircle, PlayCircle,
  SCHEDULED: Calendar,
...
export default function InterviewsTab({
  interviews, loading, searchQuery, onSearchChange, statusFilter, onStatusFilterChange,
  onSelectInterview, formatDate, formatTime, getInitials, t,
}: InterviewsTabProps) {
  const stats = {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder={t.common.search} value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} className="ps-9 h-9" />
        </div>
        <Select value={statusFilter} onValueChange={onStatusFilterChange}>
          ...
          <SelectContent>
            ...
            {interviewStatuses.map((status) => (
              <SelectItem key={status} value={status}>{getStatusLabel(status, t)}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between gap-2 mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-xs font-medium text-muted-foreground">{getTypeLabel(interview.type, t)}</p>
                          ...
                          <Badge variant="outline" className={cn('text-[10px] px-1.5 py-0 font-medium flex-shrink-0', sConfig.color, sConfig.bgColor, sConfig.borderColor)}>
                            <SIcon className="w-3 h-3 me-1" />{getStatusLabel(interview.status, t)}
                          </Badge>
                        </div>
                        <div className="space-y-2">
                          ...
                          {interviewerNames && (
                            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                              <Star className="w-3 h-3" /><span className="truncate">{interviewerNames}</span>
                            </div>
                          )}
