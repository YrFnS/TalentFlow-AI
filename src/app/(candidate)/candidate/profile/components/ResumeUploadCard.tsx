...
export default function ResumeUploadCard({
	...
	const handleUpload = async (file: File) => {
		...
		try {
			...
			const data = (await response.json()) as { file: typeof uploadedFile };
			...
			setUploadProgress(0);
			toast.error(
				error instanceof Error ? error.message : t.resume.uploadError,
			);
		} finally {
	...
	const handleDrop = (e: React.DragEvent) => {
		...
		e.stopPropagation();
		setIsDragging(false);
		// @ts-expect-error DOM FileList
		const files = e.dataTransfer.files;
		if (files.length > 0) handleUpload(files[0]);
	};
	const handleBrowseClick = () => {
		// @ts-expect-error HTMLInputElement.click() exists at runtime
								...
								<div className="min-w-0">
									...
									<div className="flex items-center gap-2 mt-0.5">
										<p className="text-xs text-muted-foreground">
											{t.resume.fileSize}: {formatFileSize(uploadedFile.size)}
										</p>
										<span className="text-xs text-muted-foreground">•</span>
										<p className="text-xs text-muted-foreground">
											{t.resume.lastUploaded}:{" "}
									...
									{parsedData.skills && parsedData.skills.length > 0 && (
										<div className="space-y-2">
											<p className="text-xs text-muted-foreground font-medium">
												{t.resume.extractedSkills} ({parsedData.skills.length})
											</p>
