...
export default function JobSearchPage() {
	...
	const load = useCallback(async (refresh = false) => {
		...
		try {
			const [jobsResponse, savedResponse, applicationsResponse] =
				await Promise.all([
			...
			if (!jobsResponse.ok) {
				throw new Error(
					await getApiErrorMessage(jobsResponse, "Unable to load jobs"),
				);
			}
			...
			setSavedIds(
				new Set(Array.isArray(saved) ? saved.map((item) => item.jobId) : []),
			);
			setApplicationByJob(
				new Map(
	...
	const filtered = useMemo(() => {
		...
		return jobs.filter((job) => {
			...
			return (
				matchesKeyword && matchesLocation && matchesType && matchesExperience
			);
		});
	...
	async function toggleSave(jobId: string) {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await getApiErrorMessage(response, "Unable to update saved job"),
				);
			}
			setSavedIds((current) => {
	...
	const filters = (
		<div className="space-y-5">
			<div className="space-y-2">
				<LabelText>{t.candidate.keyword}</LabelText>
			...
			<div className="space-y-2">
				<LabelText>{t.candidate.jobType}</LabelText>
				<Select
					value={jobType}
					onValueChange={(value) => {
						setJobType(value);
				...
				>
					<SelectTrigger aria-label={t.candidate.jobType}>
						<SelectValue />
					</SelectTrigger>
					<SelectContent>
			...
			<div className="space-y-2">
				<LabelText>{t.candidate.experienceLevel}</LabelText>
				<Select
					value={experience}
					onValueChange={(value) => {
						setExperience(value);
				...
				>
					<SelectTrigger aria-label={t.candidate.experienceLevel}>
						<SelectValue />
					</SelectTrigger>
					<SelectContent>
						<SelectItem value="all">{t.candidate.allLevels}</SelectItem>
						<SelectItem value="entry">
							{t.candidate.entryLevel} · 0–2 years
						</SelectItem>
						<SelectItem value="mid">
							{t.candidate.midLevel} · 3–5 years
						</SelectItem>
						<SelectItem value="senior">
							{t.candidate.seniorLevel} · 6+ years
						</SelectItem>
					</SelectContent>
				</Select>
			</div>
	...
	return (
		<div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6" dir={dir}>
			<div className="flex flex-wrap items-start justify-between gap-4">
				<div>
					<h1 className="text-2xl font-bold">{t.candidate.jobSearch}</h1>
					...
					<Sheet>
						<SheetTrigger asChild>
							<Button
								...
								size="sm"
								className="lg:hidden"
								aria-label={t.candidate.filters}
							>
					...
					{visibleJobs.length === 0 ? (
						<Card>
							<CardContent className="py-16 text-center">
								...
								<p className="mt-3 font-medium">{t.candidate.noJobsFound}</p>
								<p className="mt-1 text-sm text-muted-foreground">
									{t.candidate.tryDifferentFilters}
								</p>
							</CardContent>
						</Card>
					) : (
									...
									>
										<CardContent className="flex h-full flex-col p-5">
											...
											<div className="mt-4 flex flex-wrap gap-2">
												...
												{job.isRemote && (
													<Badge variant="outline">{t.jobs.remote}</Badge>
												)}
											</div>
											...
											<div className="mt-4 space-y-2 text-sm text-muted-foreground">
												<p className="flex items-center gap-2">
													...
													{job.location || t.candidate.location}
												</p>
												<p className="flex items-center gap-2">
