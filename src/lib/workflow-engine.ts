  ...
  try {
    ...
    switch (step.action) {
      case 'SEND_EMAIL': {
        ...
        const subject = (emailConfig.subject as string) || 'Notification from TalentFlow AI';
        const body = (emailConfig.body as string) || '';
        const templateKey = emailConfig.templateKey as string | undefined;

        let finalBody = body;
        if (templateKey && BUILTIN_EMAIL_TEMPLATES[templateKey as keyof typeof BUILTIN_EMAIL_TEMPLATES]) {
      ...
      case 'MOVE_STAGE': {
        ...
        if (appId && targetStageId) {
          ...
          result = { movedToStage: null, reason: 'Missing applicationId or targetStageId' };
        }
        break;
      }
        ...
        if (appId) {
          const interview = await db.interview.create({
            data: {
              ...
              scheduledAt: interviewConfig.scheduledAt ? new Date(interviewConfig.scheduledAt as string) : new Date(),
      ...
      case 'SEND_SCREENING': {
        ...
        if (jobId && questions.length > 0) {
          ...
          for (let i = 0; i < questions.length; i++) {
            ...
            const sq = await db.screeningQuestion.create({
              data: {
                jobId,
                question: q.question,
                questionType: (q.type as 'YES_NO' | 'MULTIPLE_CHOICE' | 'TEXT' | 'NUMBER' | 'DATE') || 'TEXT',
                isRequired: true,
                isKnockout: false,
                order: i,
              },
          ...
          result = { screeningQuestionsCreated: created.length, questionIds: created };
        } else {
            ...
            try {
              ...
              const aiResponse = await (sdk as any).chat?.completions?.create?.({
                ...
                messages: [
                  {
                    role: 'system',
                    content: 'You are a resume screening AI. Analyze the resume against the job requirements and return a JSON with: score (0-100), strengths (array), weaknesses (array), recommendation (string).',
                  },
                  {
                    role: 'user',
              ...
              result = { screened: true, score, threshold, passed: score >= threshold };
            } catch (aiError: unknown) {
              const errMsg = aiError instanceof Error ? aiError.message : 'AI screening failed';
        ...
        try {
          ...
          const aiResponse = await (sdk as any).chat?.completions?.create?.({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are an interview question generator. Return a JSON array of objects with: question, category, difficulty.',
              },
              {
                role: 'user',
          ...
          const errMsg = aiError instanceof Error ? aiError.message : 'AI question generation failed';
      ...
      case 'ASSIGN_RECRUITER': {
        ...
        if (appId && recruiterId) {
          ...
          result = { assigned: false, reason: 'Missing recruiterId or applicationId' };
        }
        break;
      }
      ...
      case 'SEND_NOTIFICATION': {
        ...
        const message = (notifConfig.message as string) || 'A workflow step has been executed.';
        ...
        if (url) {
          try {
            const response = await fetch(url, {
              ...
              body: JSON.stringify({ workflowId: workflow.id, executionId: execution.id, data }),
            });
            result = { webhookSent: true, statusCode: response.status };
          } catch (fetchError: unknown) {
            const errMsg = fetchError instanceof Error ? fetchError.message : 'Webhook failed';
  ...
  for (let i = execution.currentStep; i < steps.length; i++) {
    ...
    await db.workflowExecution.update({
      ...
      data: {
        currentStep: i + 1,
        stepResults: JSON.stringify(stepResults),
        status: stepResult.status === 'FAILED' ? 'FAILED' : (isLastStep ? 'COMPLETED' : 'RUNNING'),
        error: stepResult.status === 'FAILED' ? stepResult.error : null,
        completedAt: isLastStep || stepResult.status === 'FAILED' ? new Date() : null,
      },
    });
...
export const WORKFLOW_TEMPLATES = [
  {
    ...
    description: 'Automatically screen new applications with AI and move qualified candidates to the next stage.',
    trigger: 'APPLICATION_RECEIVED' as WorkflowTrigger,
    triggerConfig: JSON.stringify({}),
    steps: JSON.stringify([
      { order: 1, action: 'AI_SCREEN_RESUME', config: { threshold: 70 }, delay: 0 },
      { order: 2, action: 'CONDITION_CHECK', config: { field: 'score', operator: 'greater_than', value: 70, skipOnFail: true }, delay: 0 },
      { order: 3, action: 'MOVE_STAGE', config: { targetStageId: '' }, delay: 0 },
      { order: 4, action: 'SEND_NOTIFICATION', config: { title: 'New Qualified Application', message: 'A new application has passed AI screening.', type: 'info' }, delay: 0 },
    ]),
  ...
  },
  {
    ...
    description: 'Send a follow-up email when a candidate does not respond after a period.',
    ...
    steps: JSON.stringify([
      ...
      { order: 2, action: 'SEND_EMAIL', config: { subject: 'Following Up on Your Application', body: '<p>Hi {{candidateName}}, we wanted to follow up on your application for {{jobTitle}}.</p>' }, delay: 0 },
    ]),
    status: 'DRAFT' as WorkflowStatus,
  },
  {
    ...
    description: 'Automatically schedule an interview when a candidate reaches the interview stage.',
    trigger: 'STAGE_CHANGED' as WorkflowTrigger,
    triggerConfig: JSON.stringify({ targetStage: 'interview' }),
    steps: JSON.stringify([
      { order: 1, action: 'SCHEDULE_INTERVIEW', config: { interviewType: 'VIDEO', durationMinutes: 30 }, delay: 0 },
      { order: 2, action: 'SEND_EMAIL', config: { subject: 'Interview Scheduled', templateKey: 'interviewScheduled' }, delay: 0 },
    ]),
    status: 'DRAFT' as WorkflowStatus,
  },
  {
    ...
    triggerConfig: JSON.stringify({}),
    steps: JSON.stringify([
      { order: 1, action: 'SEND_EMAIL', config: { subject: 'Welcome Aboard!', body: '<p>Congratulations! We are thrilled to have you join the team.</p>' }, delay: 0 },
      { order: 2, action: 'SEND_NOTIFICATION', config: { title: 'Offer Accepted', message: 'A candidate has accepted the offer. Time to start onboarding!', type: 'success' }, delay: 0 },
    ]),
    status: 'DRAFT' as WorkflowStatus,
  },
];
