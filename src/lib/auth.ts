      ...
      async authorize(credentials) {
        ...
        if (!user.isActive) {
          ...
          await logAuthEvent({
            eventType: 'LOGIN_FAILURE',
            userId: user.id,
            details: { email: credentials.email, reason: 'account_deactivated' },
          });
        ...
        if (user.twoFactorEnabled) {
          ...
          if (user.twoFactorSecret) {
            try {
              ...
              const totpResult = await totp.verify(totpToken, { secret: decryptedSecret });
              ...
              if (totpResult.valid) {
                ...
                await logAuthEvent({
                  ...
                  userId: user.id,
                  details: { email: credentials.email, provider: 'credentials', method: '2fa_totp' },
                });
          ...
          if (user.backupCodes) {
            try {
              ...
              for (let i = 0; i < hashedBackupCodes.length; i++) {
                const isMatch = await bcrypt.compare(totpToken, hashedBackupCodes[i]);
                if (isMatch) {
                  ...
                  await logAuthEvent({
                    eventType: 'LOGIN_SUCCESS',
                    userId: user.id,
                    details: { email: credentials.email, provider: 'credentials', method: '2fa_backup_code' },
                  });
        ...
        if (user.email) {
          ...
          if (existingUser) {
            ...
            await logAuthEvent({
              ...
              userId: existingUser.id,
              details: { email: user.email, provider: account.provider, isNewUser: false },
            });
          } else {
            ...
            await logAuthEvent({
              eventType: 'SOCIAL_LOGIN',
              userId: newUser.id,
              details: { email: user.email, provider: account.provider, isNewUser: true },
            });
          }
        }
      }
